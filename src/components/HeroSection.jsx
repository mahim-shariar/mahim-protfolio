...
import {
  Terminal, Zap, ChevronDown, Github, Linkedin, Mail,
  ExternalLink, ArrowRight, Download, MapPin, Code2, Sparkles,
} from "lucide-react";
import { FaReact, FaNodeJs } from "react-icons/fa";
import {
  SiTypescript, SiNextdotjs, SiMongodb, SiTailwindcss,
  SiExpress, SiDocker,
} from "react-icons/si";
...
const MatrixBackground = memo(() => {
  ...
  const initMatrix = useCallback(() => {
    ...
    const ctx = canvas.getContext("2d");
    const characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz$+-*/=%\"'#&_(),.;:?!\\|{}<>[]^~ 你好世界北京上海中国文化";
    const fontSize = 14;
    const columns = Math.floor(canvas.width / dpr / fontSize);
    const rainDrops = Array(columns).fill(1);
    const colors = { primary: "#FFFFFF", secondary: "#F0F0F0", tertiary: "#A0A0A0", dark: "#202020" };
    function draw() {
  ...
  return <canvas ref={canvasRef} className="fixed inset-0 w-full h-full opacity-[0.15] pointer-events-none" style={{ zIndex: 0 }} />;
});
...
const useTypewriter = (texts, speed = 75, pauseMs = 2200) => {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      if (!deleting) {
        if (ci < current.length) { setDisplay(current.slice(0, ci + 1)); setCi((v) => v + 1); }
        else { setTimeout(() => setDeleting(true), pauseMs); }
      } else {
        if (ci > 0) { setDisplay(current.slice(0, ci - 1)); setCi((v) => v - 1); }
        else { setDeleting(false); setTi((v) => (v + 1) % texts.length); }
      }
...
const PhotoFrame = ({ src, name }) => {
  ...
  const springRotX = useSpring(rotX, { stiffness: 200, damping: 22, mass: 0.6 });
  const springRotY = useSpring(rotY, { stiffness: 200, damping: 22, mass: 0.6 });
    ...
    >
      ...
      <div className="absolute -inset-[1px] rounded-2xl overflow-hidden opacity-60">
        <motion.div
          ...
          style={{
            background: "conic-gradient(from 0deg, transparent 60%, rgba(255,255,255,0.25) 75%, transparent 90%)",
            scale: 1.5,
...
const HeroSection = () => {
  ...
  const [content, setContent] = useState({
    ...
    description: "Building scalable web applications with modern technologies. Passionate about clean code, user experience, and solving complex problems with elegant solutions.",
    resume: "",
    ...
    api.get("/content").then((res) => {
      if (res?.data) {
        setContent({
          ...
          description: res.data.description || "Building scalable web applications with modern technologies.",
          resume: res.data.resume || "",
          social: { github: res.data.social?.github || "", linkedin: res.data.social?.linkedin || "", email: res.data.social?.email || "" },
  ...
  const roleText = useTypewriter(["MERN Stack Developer", "Full Stack Engineer", "UI/UX Enthusiast", "Problem Solver"]);
  ...
  return (
    <div id="home" className="relative min-h-screen bg-black overflow-hidden flex items-center">
      ...
      <motion.div
        className="absolute pointer-events-none"
        animate={{ x: [0, -30, 15, 0], y: [0, 25, -20, 0], scale: [1, 0.9, 1.1, 1] }}
      ...
      {/* Noise grain SVG */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-[0.035] mix-blend-overlay" style={{ zIndex: 1 }}>
      ...
      {/* Radial top glow */}
      <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 40% at 50% 0%, rgba(255,255,255,0.04), transparent)" }} />
      {/* Bottom fade */}
          ...
          <div className="order-2 lg:order-1 flex flex-col">
            <motion.div {...fadeUp(0.05)} className="mb-8">
              <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full border border-white/10 bg-white/[0.03] backdrop-blur-sm">
                ...
                <span className="text-[11px] font-mono text-white/45 tracking-[0.18em] uppercase">Available for work</span>
              </div>
            </motion.div>

            <motion.div {...fadeUp(0.12)} className="mb-5">
              <h1 className="font-bold text-white leading-[0.88] tracking-tight select-none">
                <span className="block text-5xl sm:text-6xl lg:text-7xl xl:text-[5.5rem] text-white">{firstName}</span>
                <span className="block text-5xl sm:text-6xl lg:text-7xl xl:text-[5.5rem] text-white/50">{lastName}</span>
              </h1>
            </motion.div>
            ...
            <motion.div {...fadeUp(0.36)} className="flex flex-wrap gap-3 mb-10">
              ...
              >
                <motion.div
                  ...
                  style={{ background: "linear-gradient(90deg, transparent, rgba(0,0,0,0.06), transparent)", width: "40%" }}
                />
                Let's work together
              ...
              {CHIPS.map((chip, i) => {
                ...
                return (
                  <motion.div
                    ...
                    transition={{
                      opacity: { delay: 0.85 + i * 0.09, type: "spring", stiffness: 200 },
                      scale:   { delay: 0.85 + i * 0.09, type: "spring", stiffness: 200 },
